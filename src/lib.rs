...
///
/// e.g
/// add_thread(&mut cList,&mut visited, 0, &program);
/// add_thread(&mut nList,&mut visited, pc + 1, &program);
...
pub fn run(program: &[Instructions], input: &str) -> bool {
    let mut cList: Vec<usize> = vec![]; // threads for curr char
    let mut nList: Vec<usize> = vec![]; // threads being built for next char

    {
        let mut visited: Vec<usize> = vec![];

        // Add initial pc(s) in cList at the very start of the program to process later
        add_thread(&mut cList, &mut visited, 0, &program);
    }
    ...
    for sp in 0..=input.len() {
        ...
        for &pc in &cList {
            if let Instructions::Char(expected) = program[pc] {
                if input.as_bytes().get(sp) == Some(&expected) {
                    add_thread(&mut nList, &mut visited, pc + 1, &program);
                }
            }
        }
        // Iteration done, update cList, empty out nList to prepare for the next iteration.
        cList = nList;
        nList = vec![];
